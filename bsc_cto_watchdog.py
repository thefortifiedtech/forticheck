...
def save_watchlist(active_tokens, chain='bsc'):
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
...
def watchdog_cycle():
    ...
    for mint, data in active_tokens.items():
        ...
        if current_smart >= initial_smart + 2 and current_smart > data.get("last_alerted_smart_money", 0):
            ...
            send_discord_message(f"BSC Watchdog: {data.get('symbol')}", embed, 'bsc')
    ...
    # Save state
    save_watchlist(active_tokens, 'bsc')

if __name__ == "__main__":
    print("🚀 BSC CTO Watchdog Background Worker Started")
    while True:
        watchdog_cycle()
